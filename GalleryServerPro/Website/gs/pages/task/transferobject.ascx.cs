...
namespace GalleryServerPro.Web.gs.pages.task
{
	public partial class transferobject : Pages.TaskPage
	{
		...
		protected static string GetAlbumText(string title, Type galleryObjectType)
		{
			...
			string titleText;
			if (truncatedText.Length != title.Length)
				titleText = string.Format(CultureInfo.CurrentCulture, "<p class=\"albumtitle gsp_nowrap\"><b>{0}</b> {1}...</p>", Resources.GalleryServerPro.Task_Transfer_Objects_Album_Title_Prefix_Text, truncatedText);
			else
				titleText = string.Format(CultureInfo.CurrentCulture, "<p class=\"albumtitle gsp_nowrap\"><b>{0}</b> {1}</p>", Resources.GalleryServerPro.Task_Transfer_Objects_Album_Title_Prefix_Text, truncatedText);
		...
		private void btnOkClicked(string step)
		{
			...
			else if (step == "step2")
			{
				try
				{
					...
					{
						Util.Redirect(PageId.album, "aid={0}&msg={1}", this.DestinationAlbum.Id.ToString(CultureInfo.InvariantCulture), ((int)Message.ObjectsSuccessfullyCopied).ToString(CultureInfo.InvariantCulture));
					...
					{
						Util.Redirect(PageId.album, "aid={0}&msg={1}", this.DestinationAlbum.Id.ToString(CultureInfo.InvariantCulture), ((int)Message.ObjectsSuccessfullyMoved).ToString(CultureInfo.InvariantCulture));
						//string[] queryStringNames = new string[] { "aid", "msg" };
