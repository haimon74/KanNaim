...
var _widthBorderBuffer = 40; // Space to allow for margins, padding, and border on media object. Also defined in controls/mediaobjectview.ascx.cs
...
{
	Gsp.Gallery.SetMetaDataVisibility(showMetadata);
}
...
function showCurrentMediaObject()
{
	...
	if (_tbIsVisible)
	{
		...
		if (tbiHiRes)
		{
			...
			{
				$get('divMoView').style.width = _moInfo.Width + _widthBorderBuffer + "px";
			}
		}
...
function showNextMediaObject()
{
	...
	if (_tbIsVisible)
	{
		...
		if (tbiHiRes)
		{
			...
			if (tbiHiRes.get_checked())
			{
				$get('divMoView').style.width = (_ssInProgress ? _moInfo.NextSSWidth : _moInfo.NextWidth) + _widthBorderBuffer + "px";
			}
		}
...
function showPrevMediaObject()
{
	...
	if (_tbIsVisible)
	{
		...
		if (tbiHiRes)
		{
			...
			if (tbiHiRes.get_checked())
			{
				$get('divMoView').style.width = _moInfo.PrevWidth + _widthBorderBuffer + "px";
			}
		}
