...
namespace GalleryServerPro.Web
{
	...
	public class MediaObjectHtmlBuilder : IMediaObjectHtmlBuilder
  {
    ...
    #region Constructors
		...
		public MediaObjectHtmlBuilder(int mediaObjectId, int albumId, IMimeType mimeType, string mediaObjectPhysicalPath, int width, int height, string title, Array browsers, DisplayObjectType displayType, bool isPrivate)
		{
			#region Validation
		...
		public bool AutoStartMediaObject
		{
			get
			{
				...
				{
					this._autoStartMediaObject = Config.GetCore().AutoStartMediaObject;
				}
		...
		public string GenerateHtml()
		{
			...
			bool autoStartMediaObject = Config.GetCore().AutoStartMediaObject;
		...
		private string ReplaceMediaObjectAbsoluteUrlNoHandlerParameter(string htmlOutput)
		{
			...
			string directUrl = String.Concat(Util.AppRoot, "/", relativePath.Replace("\\", "/"));
		...
		{
			MediaObject mocfg = Config.GetGalleryObject().MediaObjects.FindByMimeType(this.MimeType.FullType);

			if (mocfg == null)
			{
				mocfg = Config.GetGalleryObject().MediaObjects.FindByMimeType(string.Format(CultureInfo.CurrentCulture, "{0}/*", this.MimeType.MajorType.ToString().ToLowerInvariant()));
			}
		...
		{
			string[] browserCompatibleImageTypes = Config.GetCore().ImageTypesStandardBrowsersCanDisplay.ToUpperInvariant().Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
		...
		/// <param name="albumId">The unique identifier for album containing the media object.</param>
		/// <param name="mimeType">The MIME type of this media object.</param>
		/// <param name="mediaObjectPhysicalPath">The physical path to this media object, including the object's name. Example:
		/// C:\Inetpub\wwwroot\galleryserverpro\mediaobjects\Summer 2005\sunsets\desert sunsets\sonorandesert.jpg</param>
		/// <param name="displayType">The type of the display object.</param>
		/// <param name="isPrivate">if set to <c>true</c>, specifies the media object is marked as private. Private albums and media
		/// objects are hidden from anonymous (unauthenticated) users.</param>
		/// <returns>Gets the URL to the media object.</returns>
