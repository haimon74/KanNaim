...
namespace GalleryServerPro.Business
{
	...
	public class Watermark : IDisposable
	{
		...
		/// <summary>
		/// Gets a reference to the <see cref="Watermark" /> singleton instance for this app domain.
		/// </summary>
		/// <value>The <see cref="Watermark" /> singleton instance.</value>
			...
			get
			{
				if (_instance == null)
				{
					lock (sharedLock)
					{
						if (_instance == null)
						{
							Watermark tempWatermark = AppSetting.Instance.IsInReducedFunctionalityMode ? GetReducedFunctionalityModeWatermark() : GetUserSpecifiedWatermark();

							// Ensure that writes related to instantiation are flushed.
							System.Threading.Thread.MemoryBarrier();
							_instance = tempWatermark;
						}
					}
				}
		...
		private static ImageAttributes GetWatermarkImageAttributes(int imageOpacityPercent)
		{
