...
namespace GalleryServerPro.Business
{
  ...
  public class Album : GalleryObject, IAlbum
  {
    ...
    #region Constructors
		...
		/// </summary>
		internal Album() : this(int.MinValue)
		{
		}
  	...
  	internal Album(int albumId)
  		: this(albumId, int.MinValue, string.Empty, string.Empty, string.Empty, int.MinValue, int.MinValue, DateTime.MinValue, DateTime.MinValue, String.Empty, DateTime.MinValue, String.Empty, DateTime.MinValue, String.Empty, String.Empty, false)
  	{
  	}
  	...
  	public override string FullPhysicalPath
  	{
  		get
  		{
  			...
  			if (this.IsRootAlbum)
  			{
  				...
  				{
  					this._fullPhysicalPathOnDisk = AppSetting.Instance.MediaObjectPhysicalPath;
  				}
		...
		public override string FullPhysicalPathOnDisk
		{
			get
			{
				...
				System.Diagnostics.Debug.Assert(false, string.Format(CultureInfo.CurrentCulture, "Should never get to this point (IsNew={0}, IsInflated={1})", this.IsNew, this.IsInflated));
				throw new BusinessException("Invalid object state. Album.IsNew = false, Album.IsInflated = true, and the private member variable _fullPhysicalPathOnDisk is either null or empty.");
			}
			set
			{
		...
		public override void Remove(IGalleryObject galleryObject)
		{
			if (galleryObject == null)
				throw new System.ArgumentNullException("galleryObject");

			if (!this._galleryObjects.Contains(galleryObject))
				throw new System.ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.Album_Remove_Ex_Msg, this.Id, galleryObject.Id, galleryObject.Parent.Id));
		...
		public override void MoveTo(IAlbum destinationAlbum)
		{
			...
			IGalleryObjectCollection albumsNeedingNewThumbnails = ValidateAlbumThumbnailsForMove(this.ThumbnailMediaObjectId, destinationAlbum.Id);

			// Step 2: Assign the new parent album to this album and save.
		...
		/// The owner user name and the owner role name is not copied.
		/// </summary>
		/// <returns>
		/// Returns a reference to a new object that is a copy of the current gallery object.
		/// </returns>
		public override IGalleryObject Copy()
		{
			IAlbum albumCopy = Factory.CreateAlbumInstance();
		...
		/// </summary>
		/// <param name="album">The the album whose thumbnail image is to be assigned.</param>
		...
		{
			int maxLength = GalleryServerPro.Configuration.ConfigManager.GetGalleryServerProConfigSection().DataStore.AlbumTitleLength;
		...
		{
			int maxLength = GalleryServerPro.Configuration.ConfigManager.GetGalleryServerProConfigSection().DataStore.AlbumSummaryLength;
		...
		private void ValidateDirectoryName()
		{
			...
			if (String.IsNullOrEmpty(this.DirectoryName))
			{
				...
				string newDirName = HelperFunctions.ValidateDirectoryName(dirPath, dirName);

				if (this.DirectoryName != newDirName)
				{
					this.DirectoryName = newDirName;
				}
			}

			int maxLength = GalleryServerPro.Configuration.ConfigManager.GetGalleryServerProConfigSection().DataStore.AlbumDirectoryNameLength;
			if (this.DirectoryName.Length > maxLength)
				throw new BusinessException(String.Format("Invalid directory name. The maximum length for a directory name is {0} characters, but one was specified that is {1} characters. More info: album ID = {2}; album title = '{3}'", maxLength, this.DirectoryName.Length, this.Title));
		}

		private IDisplayObject GetDefaultAlbumThumbnail()
		{
			string defaultFilename = GalleryServerPro.Business.GlobalConstants.DefaultFileName;
			int maxLength = GalleryServerPro.Configuration.ConfigManager.GetGalleryServerProConfigSection().Core.MaxThumbnailLength;
			float ratio = GalleryServerPro.Configuration.ConfigManager.GetGalleryServerProConfigSection().Core.EmptyAlbumThumbnailWidthToHeightRatio;
		...
		{
			//if (String.IsNullOrEmpty(this.OwnerUserName))
			//{
			//  this.OwnerUserName = this.CreatedByUserName;
			//}
		}
