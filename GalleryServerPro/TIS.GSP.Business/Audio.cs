...
namespace GalleryServerPro.Business
{
	...
	public class Audio : GalleryObject
	{
		...
		internal Audio(System.IO.FileInfo audioFile, IAlbum parentAlbum)
		...
		/// <remarks>This constructor does not verify that <paramref name="audioFile"/> refers to a file type that is enabled in the 
		/// configuration file.</remarks>
		internal Audio(int id, int parentId, IAlbum parentAlbum, string title, string hashKey, string thumbnailFilename, int thumbnailWidth, int thumbnailHeight, int thumbnailSizeKb, string originalFilename, int originalWidth, int originalHeight, int originalSizeKb, int sequence, string createdByUsername, DateTime dateAdded, string lastModifiedByUsername, DateTime dateLastModified, bool isPrivate, bool isInflated, FileInfo audioFile)
		{
			...
			if (audioFile != null)
			{
				...
				if (String.IsNullOrEmpty(title)) this.Title = audioFile.Name;
		...
		public override void Inflate()
		{
			...
			if ((!this.IsNew) && (!this.IsInflated))
			{
				...
				if ((!this.IsInflated) || (this.HasChanges))
					throw new System.InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resources.Audio_Inflate_Ex_Msg, this.IsInflated, this.HasChanges));
			}
		}
