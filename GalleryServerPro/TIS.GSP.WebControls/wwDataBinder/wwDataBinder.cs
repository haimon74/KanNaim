...
namespace GalleryServerPro.WebControls
{
	...
	public class wwDataBinder : Control, IExtenderProvider  //System.Web.UI.WebControls.WebControl
	{
		...
		protected string ErrorIconWebResource
		{
			get
			{
				if (string.IsNullOrEmpty(this._ErrorIconWebResource))
					this._ErrorIconWebResource = Page.ClientScript.GetWebResourceUrl(this.GetType(), "GalleryServerPro.WebControls.Resources.warning.gif");
		...
		public bool Unbind(Control Container)
		{
			...
			foreach (IValidator Validator in this.Page.Validators)
			{
				...
				if (BValidator != null)
				{
					Control Ctl = wwWebUtils.FindControlRecursive(this.Page, BValidator.ControlToValidate);
					if (Ctl != null)
...
if (Sibling != null)
 ...
 Ctl.parentNode.appendChild(Insert);
	...
	/// </summary>
	internal class wwDataBinderDesigner : ControlDesigner
	{
