...
{
	public partial class albumtreeview : System.Web.UI.UserControl
	{
		...
		/// <summary>
		/// Gets or sets the security permission the logged on user must have in order for the checkbox
		/// to be rendered in the treeview.  It may be a single value or some
		/// combination of valid enumeration values.
		/// </summary>
		...
		/// Ex: "&secaction=3"
		/// </summary>
		protected string SecurityPermissionQueryStringParm
		{
			get
			{
		...
		public void DataBind(IAlbum albumToSelect)
		{
		...
		{
			cv.Text = String.Format(CultureInfo.CurrentCulture, "<span class='bold'>{0} </span>{1}", Resources.GalleryServerPro.Task_Transfer_Objects_Cannot_Transfer_No_Destination_Album_Selected_Hdr, Resources.GalleryServerPro.Task_Transfer_Objects_Cannot_Transfer_No_Destination_Album_Selected_Dtl);
		...
		private void bindTreeView()
		{
			...
			// Add root node.
			IAlbum rootAlbum = Factory.LoadViewableRootAlbumByRoles(this.PageBase.GetGalleryServerRolesForUser());

			TreeViewNode rootNode = new TreeViewNode();
			rootNode.Text = WebsiteController.RemoveHtmlTags(rootAlbum.Title);
			...
			bool isUserAuthorized = (IsSecurityPermissionRequested() ? this.PageBase.IsUserAuthorized(this.RequiredSecurityPermissions, rootAlbum) : true);
			rootNode.ShowCheckBox = (!rootAlbum.IsVirtualAlbum && isUserAuthorized);
			rootNode.Selectable = (!rootAlbum.IsVirtualAlbum && isUserAuthorized);
			if (!isUserAuthorized) rootNode.HoverCssClass = String.Empty;

			// Select and check this node if needed.
			if ((this._albumToSelect != null) && (rootAlbum.Id == _albumToSelect.Id))
			{
		...
		/// <summary>
		/// Gets a value indicating whether the RequiredSecurityPermission has been assigned a valid value.
		/// </summary>
		/// <returns>Returns a value indicating whether the RequiredSecurityPermission has been assigned a valid value.</returns>
		private bool IsSecurityPermissionRequested()
		{
			return SecurityActionEnumHelper.IsValidSecurityAction(this.RequiredSecurityPermissions);
		}
...
{{
	...
	// Enforce rules: (1) An album can never be unchecked when its parent is checked.
	// (2) Checking or unchecking nodes should propagate to all its children.
	var selectedNode = eventArgs.get_node(); 
	if (selectedNode == null) return;

	if (selectedNode.get_checked() == true)
	{{
		var parentNode = selectedNode.get_parentNode();
		if ((parentNode == null) || ((parentNode != null) && (!parentNode.get_checked())))
		{{
...
function uncheckAll(parentNode)
{{
	parentNode.set_checked(false);
	var nodes = parentNode.get_nodes();
	for (var i = 0; i < nodes.get_length(); i++)
	{{
		uncheckAll(nodes.getNode(i));
	}}
}}
...
{{
	...
	{{
		// Since the 'before' state is checked, user is trying to uncheck the node. Allow this only
		// when the parent node is checked.
		var parentNode = checkedNode.get_parentNode();
		if ((parentNode != null) && (parentNode.get_checked()))
			eventArgs.set_cancel(true); // Parent node is checked - cancel event!
		else
			uncheckAll(checkedNode); // Ok to uncheck - propagate to all children, too.
	}}
	else
...
{{
	...
	sender.render();
			...
			System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, typeof(GspPage), "tvFunctions", script, true);
