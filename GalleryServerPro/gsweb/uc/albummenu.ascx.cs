...
{
	public partial class albummenu : System.Web.UI.UserControl
	{
		...
		public bool ShowActionMenu
		{
			get
			{
				...
				{
					if (!this.PageBase.User.Identity.IsAuthenticated)
					{
					...
					else
					{
						...
						return (this.PageBase.UserCanAdministerSite ||
							this.PageBase.UserCanEditMediaObject ||
							this.PageBase.UserCanEditAlbum ||
							this.PageBase.UserCanDeleteCurrentAlbum ||
							this.PageBase.UserCanDeleteMediaObject ||
							this.PageBase.UserCanSynchronize ||
							this.PageBase.UserCanAddAlbumToAtLeastOneAlbum ||
							this.PageBase.UserCanAddMediaObjectToAtLeastOneAlbum);
					}
				}
		...
		{
			if (this.ShowActionMenu)
				phActionMenu.Controls.Add(Page.LoadControl("~/uc/actionmenu.ascx"));
		}

		private string buildMenuString()
		{
			string menuString = string.Empty;
			string appPath = WebsiteController.GetAppRootUrl();

			IAlbum album = PageBase.GetAlbum();
			...
			while (!foundTopAlbum)
			{
				...
				else if (!GspPage.IsUserAuthorized(SecurityActions.ViewAlbumOrMediaObject, roles, album.Id, album.IsPrivate))
				{
					...
					IAlbum rootAlbum = Factory.LoadViewableRootAlbumByRoles(PageBase.GetGalleryServerRolesForUser());
				...
				else
				{
					...
					menuString = menuString.Insert(0, string.Format(CultureInfo.CurrentCulture, " {0} <a{1} href=\"{2}?aid={3}\">{4}</a>", dividerText, hyperlinkIdString, appPath, album.Id, WebsiteController.RemoveHtmlTags(album.Title)));
				}
