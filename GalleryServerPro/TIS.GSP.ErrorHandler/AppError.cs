	...
	public class AppError : IAppError, IComparable
	{
		...
		public string HttpUserAgent
		{
			get
			{
				...
				{
					return (String.Compare(kvp.Key, "HTTP_USER_AGENT", true) == 0);
				});
...
";
		...
		internal AppError(Exception ex)
		{
			this._appErrorId = int.MinValue;
			this._galleryId = ConfigManager.GetGalleryServerProConfigSection().Core.GalleryId;
			this._timeStamp = DateTime.Now;

			string missingDataText = Resources.Err_Missing_Data_Txt;

			this._exceptionType = (ex.GetType() == null) ? missingDataText : ex.GetType().ToString();
			...
			if (ex.InnerException != null)
			{
				...
				this._innerExType = (innerEx.GetType() == null) ? missingDataText : innerEx.GetType().ToString();
		...
		public string ToHtmlValue(ErrorItem item)
		{
			switch (item)
			{
				case ErrorItem.AppErrorId: return ToHtmlParagraph(this.AppErrorId.ToString());
				case ErrorItem.Url: return ToHtmlParagraph(this.Url);
				case ErrorItem.Timestamp: return ToHtmlParagraph(this.TimeStamp.ToString());
				...
				case ErrorItem.GalleryId: return ToHtmlParagraph(this.GalleryId.ToString());
				...
				default: throw new Exception(String.Format("Encountered unexpected ErrorItem enum value {0}. AppError.ToHtmlValue() is not designed to handle this enum value. The function must be updated.", item));
			}
		}
		...
		private string ToHtmlTable(ErrorItem item)
		{
			...
			switch (item)
			{
				...
				default: throw new Exception(String.Format("Encountered unexpected ErrorItem enum value {0}. AppError.ToHtmlTable() is not designed to handle this enum value. The function must be updated.", item));
			}
...
<table cellpadding='0' cellspacing='0' class='gsp_err_table'>
 <tr><td>{0}</td></tr>
			...
			else
			{
				...
				foreach (KeyValuePair<string, string> pair in list)
				{
					listString += String.Format("<tr><td class='gsp_err_col1'>{0}</td><td class='gsp_err_col2'>{1}</td></tr>\n", ToHtmlParagraph(pair.Key), ToHtmlParagraph(MakeHtmlLineWrapFriendly(pair.Value)));
				}
				listString += "</table>";
