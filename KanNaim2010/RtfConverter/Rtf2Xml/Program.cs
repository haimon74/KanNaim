...
namespace Itenso.Solutions.Community.Rtf2Xml
{
	...
	class Program
	{
		...
		public void Execute()
		{
			...
			Console.WriteLine( "successfully converted RTF to XML in " + this.settings.DestinationDirectory );
		} // Execute
		...
		private void ConvertXml( IRtfDocument rtfDocument )
		{
			try
			{
				...
				xmlWriterSettings.Encoding = this.settings.Encoding;

				string fileName = this.settings.BuildDestinationFileName( null, RtfXmlConverter.DefaultXmlFileExtension );
				using ( XmlWriter writer = XmlWriter.Create( fileName, xmlWriterSettings ) )
				{
					if ( writer != null )
					{
						RtfXmlConvertSettings xmlConvertSettings = new RtfXmlConvertSettings();
						xmlConvertSettings.Prefix = this.settings.XmlPrefix;
						xmlConvertSettings.Ns = this.settings.XmlNamespace;
						xmlConvertSettings.IsShowHiddenText = this.settings.ShowHiddenText;
