...
namespace Itenso.Solutions.Community.Rtf2Raw
{
	...
	class Program
	{
		...
		public void Execute()
		{
			...
			Console.WriteLine( "successfully converted RTF to Raw data in " + this.settings.DestinationDirectory );
		} // Execute
		...
		private string InterpretRtf( IRtfGroup rtfStructure )
		{
			...
			try
			{
				...
				if ( this.settings.SaveText )
				{
					RtfTextConvertSettings textConvertSettings = new RtfTextConvertSettings();
					textConvertSettings.IsShowHiddenText = this.settings.ShowHiddenText;
				...
				if ( this.settings.SaveImage )
				{
					...
					imageConvertSettings.ScaleImage = this.settings.ScaleImage;
		...
		private string SaveText( string text )
		{
			...
			string fileName = this.settings.BuildDestinationFileName( null, RtfTextConverter.DefaultTextFileExtension );
			try
			{
		...
		{
			if ( !this.settings.SaveText || !this.settings.OpenTextFile )
			{
				return;
			}
		...
		{
			if ( !this.settings.DisplayRawText )
			{
				return;
			}
